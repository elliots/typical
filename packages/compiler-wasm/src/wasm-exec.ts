  ...
  constructor() {
    ...
    const loadString = (addr: number): string => {
      ...
      return decoder.decode(
        new DataView((this._inst.exports as unknown as GoExports).mem.buffer, saddr, len),
      );
    };
      ...
      gojs: {
        ...
        "runtime.wasmWrite": (sp: number) => {
          ...
          const data = new Uint8Array(
            (this._inst.exports as unknown as GoExports).mem.buffer,
            p,
            n,
          );
        ...
        "syscall/js.valueInstanceOf": (sp: number) => {
          ...
          this.mem.setUint8(
            sp + 24,
            (loadValue(sp + 8) as object) instanceof
              (loadValue(sp + 16) as new (...args: any[]) => any)
              ? 1
              : 0,
          );
        },
