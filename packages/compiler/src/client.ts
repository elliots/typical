...
export class TypicalCompiler {
  ...
  private handleData(data: Buffer): void {
    ...
    debugLog(
      `[CLIENT DEBUG] handleData: received ${data.length} bytes, buffer now ${this.buffer.length} bytes`,
    );
    ...
    while (this.buffer.length > 0) {
      try {
        ...
        debugLog(
          `[CLIENT DEBUG] Decoded: type=${messageType} method=${method} payload=${payload.length} bytes, consumed=${bytesConsumed}`,
        );
        ...
        if (!pending) {
          ...
          throw new Error(
            `No pending request for method: ${method}. Pending requests: ${pendingKeys}. ` +
