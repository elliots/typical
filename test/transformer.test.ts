...
void describe('TypicalTransformer', () => {
  ...
  const testCases: TestCase[] = [
    {
      ...
      expectedPatterns: [`import typia from "typia"`, `const __typical_assert_string = typia.createAssert<string>()`, `__typical_assert_string(name)`, `return __typical_assert_string("Hello " + name)`],
    },
    {
...
function processData(data: User) {
  return JSON.stringify(data);
      ...
      expectedPatterns: [`import typia from "typia"`, `typia.json.createStringify<User>()`],
...
function getData(): { id: number, name: string } {
  ...
  return data;
      ...
      expectedPatterns: [`import typia from "typia"`, `typia.createAssert<{`, `id: number`, `name: string`, `return __typical_assert_0(data)`],
    },
    {
...
const multiply = (a: number, b: number): number => {
  return a * b;
      ...
      expectedPatterns: [`import typia from "typia"`, `const __typical_assert_number = typia.createAssert<number>()`, `__typical_assert_number(a)`, `__typical_assert_number(b)`, `return __typical_assert_number(a * b)`],
    },
    {
      name: 'complex types',
      input: `
...
const fn = () => (() => JSON.stringify(user))();`,
      expectedPatterns: [`typia.json.createStringify<User>()`, `const fn = () => (() => __typical_stringify_User(user))()`],
      notExpectedPatterns: [`JSON.stringify`],
    },
    {
...
const fn = (s: string): User => JSON.parse(s);`,
      expectedPatterns: [`typia.json.createAssertParse<User>()`, `=> __typical_parse_User`],
      notExpectedPatterns: [`JSON.parse`],
    },
...
const getData = (): Promise<Data> => fetchData();`,
      expectedPatterns: [`typia.createAssert<Data>()`, `.then(__typical_assert_Data)`],
      notExpectedPatterns: [`await fetchData`],
    },
...
function process(data: VeryLongTypeName): VeryLongTypeName {
  return data;
      ...
      expectedPatterns: [`typia.createAssert<VeryLongTypeName>()`],
      notExpectedPatterns: [
...
function updateUser(partial: Partial<User>): Partial<User> {
  return partial;
      ...
      expectedPatterns: [`typia.createAssert<Partial<User>>()`, `__typical_assert_`],
    },
    {
...
function getName(user: Pick<User, "name">): Pick<User, "name"> {
  return user;
      ...
      expectedPatterns: [`typia.createAssert<Pick<User, "name">>()`],
    },
...
function pet(animal: Cat | Dog): Cat | Dog {
  return animal;
      ...
      expectedPatterns: [`typia.createAssert<Cat | Dog>()`, `__typical_assert_`],
    },
    {
...
function process(person: Named & Aged): Named & Aged {
  return person;
      ...
      expectedPatterns: [`typia.createAssert<Named & Aged>()`, `__typical_assert_`],
    },
    {
...
function area(shape: Shape): number {
  return shape.kind === "circle" ? 3.14 * shape.radius ** 2 : shape.side ** 2;
      ...
      expectedPatterns: [`typia.createAssert<Shape>()`, `__typical_assert_Shape(shape)`],
    },
    // Class method tests
    {
...
class UserService {
  getUser(id: number): User {
    return { name: "test" };
      ...
      expectedPatterns: [`typia.createAssert<number>()`, `typia.createAssert<User>()`, `__typical_assert_number(id)`],
    },
    {
...
class AppConfig {
  static parse(json: string): Config {
    return { debug: true };
      ...
      expectedPatterns: [`typia.createAssert<string>()`, `typia.createAssert<Config>()`, `__typical_assert_string(json)`],
    },
...
function add(a: number): (b: number) => number {
  return (b: number) => a + b;
      ...
      expectedPatterns: [`typia.createAssert<number>()`, `__typical_assert_number(a)`],
      notExpectedPatterns: [
...
function add(a: number): (b: number) => number {
  return (b: number) => {
    return a + b;
      ...
      expectedPatterns: [`typia.createAssert<number>()`, `__typical_assert_number(a)`, `__typical_assert_number(b)`],
    },
  ]
  ...
  void describe('validateCasts option', () => {
    void test("transforms 'as' casts when validateCasts is enabled", () => {
      const fileName = 'test/test.temp.ts'
      const input = `
interface User {
  name: string;
...
`
      ...
      assert.ok(transformedCode.includes('typia.createAssert<User>()'), 'Should create validator for User type')
      // Should wrap the cast
      assert.ok(transformedCode.includes('__typical_assert_'), 'Should replace cast with validator call')
      // Should NOT have 'as User'
      assert.ok(!transformedCode.includes('as User'), "Should not have 'as User' cast in output")
    })

    void test('does not transform casts when validateCasts is disabled (default)', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
interface User {
  name: string;
...
`
      ...
      assert.ok(!transformedCode.includes('__typical_assert_'), 'Should not add validator when validateCasts is false')
    })

    void test("skips 'as any' casts", () => {
      const fileName = 'test/test.temp.ts'
      const input = `
...
`
      ...
      assert.ok(!transformedCode.includes('__typical_assert_'), "Should not validate 'as any' casts")
    })

    void test("skips 'as unknown' casts", () => {
      const fileName = 'test/test.temp.ts'
      const input = `
const data = { name: "test" };
...
`
      ...
      assert.ok(!transformedCode.includes('__typical_assert_'), "Should not validate 'as unknown' casts")
    })
  })

  // Pre-compiled ignore pattern tests
  void describe('ignoreTypes pattern compilation', () => {
    void test('compiles valid glob patterns to RegExp', () => {
    ...
    void test('compiles wildcard-only pattern', () => {
    ...
    void test('compiles exact match pattern', () => {
    ...
    void test('compileIgnorePatterns creates all pattern categories', () => {
      ...
      assert.strictEqual(compiled.allPatterns.length, compiled.userPatterns.length + compiled.domPatterns.length, 'allPatterns should be sum of user and DOM patterns')
    })

    void test('compileIgnorePatterns respects ignoreDOMTypes: false', () => {
      const config: TypicalConfig = {
        ignoreTypes: ['MyType'],
    ...
    void test('ignoreTypes patterns are used during transformation', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
...
function process(x: MyIgnoredType): MyIgnoredType {
  return x;
      ...
      assert.ok(!transformedCode.includes('typia.createAssert<MyIgnoredType>'), 'Should not create validator for ignored type')
    })

    void test('wildcard ignoreTypes pattern matches multiple types', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
...
`
      ...
      assert.ok(!transformedCode.includes('typia.createAssert<MyEvent>'), 'Should not create validator for MyEvent')
      assert.ok(!transformedCode.includes('typia.createAssert<OtherEvent>'), 'Should not create validator for OtherEvent')
    })
  })
  ...
  void describe('full transpile (typia mode)', () => {
    void test('transforms simple primitive validation with typia', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function checkNumber(n: number): number {
  return n;
      ...
      assert.ok(!transformedCode.includes('typia.createAssert<'), 'typia.createAssert<T>() calls should be transformed away')
      // Should have typia's internal validation imports
      assert.ok(transformedCode.includes('__typia_transform__'), 'Should contain typia internal imports')
    })

    void test('hoistRegex: false does not add __regex_ variables', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function checkNumber(n: number): number {
  return n;
      ...
      assert.ok(!transformedCode.includes('const __regex_'), 'Should not hoist regex when hoistRegex is false')
    })

    void test('reusableValidators: false generates inline validation', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function checkString(s: string): string {
  return s;
      ...
      assert.ok(!transformedCode.includes('const __typical_assert_'), 'Should not have reusable validator variable declarations')
      // Should still have typia's validation logic
      assert.ok(transformedCode.includes('__typia_transform__'), 'Should contain typia validation')
    })

    void test('validateFunctions: false skips all function validation', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function noValidation(n: number): number {
  return n;
      ...
      assert.ok(!transformedCode.includes('__typical_assert_'), 'Should not have validators when validateFunctions is false')
      assert.ok(!transformedCode.includes('__typia_transform__'), 'Should not have typia imports when validateFunctions is false')
    })

    void test('multiple primitive parameters all get validated', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function multiParam(a: string, b: number, c: boolean): string {
  return a;
      ...
      assert.ok(
        transformedCode.includes('__typical_assert_string') && transformedCode.includes('__typical_assert_number') && transformedCode.includes('__typical_assert_boolean'),
        'Should have validators for all primitive types',
      )
    })

    void test('interface types are fully transformed by typia', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
interface User {
  name: string;
...
function validateUser(user: User): User {
  return user;
      ...
      assert.ok(!transformedCode.includes('typia.createAssert<'), 'typia.createAssert<T>() should be transformed')
      // Should contain property validation for User
      assert.ok(transformedCode.includes('"string" === typeof input.name') || transformedCode.includes('input.name'), 'Should validate name property')
      assert.ok(transformedCode.includes('"number" === typeof input.age') || transformedCode.includes('input.age'), 'Should validate age property')
    })

    void test('array types are transformed by typia', () => {
      const fileName = 'test/test.temp.ts'
      const input = `
function processNumbers(nums: number[]): number[] {
  return nums;
      ...
      assert.ok(transformedCode.includes('Array.isArray'), 'Should contain Array.isArray check')
    })
  })

  // Source map generation tests
  void describe('source map generation', () => {
    void test('generates source map when sourceMap option is true', () => {
    ...
    void test('does not generate source map when sourceMap option is false', () => {
    ...
    void test('source map includes content when includeContent is true', () => {
    ...
    void test('every line in output has a source mapping', () => {
...
`
      ...
      outputLines.forEach((line, idx) => {
        ...
        if (trimmed.length > 0 && !trimmed.startsWith('//# sourceMappingURL')) {
          assert.ok(mappedLines.has(lineNum), `Line ${lineNum} should have a source mapping: "${line.substring(0, 50)}..."`)
        }
      })
    })

    void test('validation code maps to type annotation position', () => {
      ...
      const validationLineIdx = outputLines.findIndex(line => line.includes('__typical_assert_string(name)'))
    ...
    void test('multiple parameters map to their type annotations', () => {
      ...
      const validationALineIdx = outputLines.findIndex(line => line.includes('__typical_assert_number(a)') || line.includes('typia.assert<number>(a)'))
      ...
      const validationBLineIdx = outputLines.findIndex(line => line.includes('__typical_assert_number(b)') || line.includes('typia.assert<number>(b)'))
    ...
    void test('return validation maps to return type annotation', () => {
      ...
      const returnLineIdx = outputLines.findIndex(line => line.includes('return __typical_assert_string('))
    ...
    void test('generated preamble lines map to type annotation positions', () => {
    ...
    void test('typia mode source maps point to original source', () => {
      ...
      assert.ok(unmappedLines.length === 0, `All lines should be mapped. Unmapped: ${unmappedLines.join(', ')}`)
    })

    void test('js mode source maps compose correctly', () => {
      ...
      assert.ok(result.map!.sourcesContent, 'Should have sourcesContent')
      assert.ok(result.map!.sourcesContent![0]?.includes('number'), 'sourcesContent should have original TypeScript')
    })
  })
})
