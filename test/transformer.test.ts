...
void describe("TypicalTransformer v2", () => {
  ...
  void test("should transform a simple function with parameter validation", async () => {
    ...
    assert.ok(
      result.code.length > fixtures.simple.code.length,
      "Output should be longer than input (validation added)",
    );
  });
  ...
  void test("should transform function with union type", async () => {
    ...
    assert.ok(
      result.code.includes("string") || result.code.includes("number"),
      "Should contain type checks",
    );
  });

  void test("should throw error for unsupported js mode", async () => {
    await assert.rejects(
      () => transformer.transform(fixtures.simple.file, "js"),
      /Mode "js" not yet supported/,
      "Should throw for js mode",
    );
  });
});
